...
make.decision.CFO.fn <- function(phi, cys, cns, alp.prior, bet.prior, cover.doses, diag=FALSE){
  if (cover.doses[2] == 1){
    ...
    if (is.na(cys[1]) & (cover.doses[3]==1)){
      ...
      #print(data.frame("cfo right OR"=OR.v2, "cfo right gamma"=gam2, "cfo right ratio"=OR.v2/gam2))
      if (OR.v2>gam2){
        return(3)
      }else{
        return(2)
      ...
      #print(data.frame("cfo left OR"=OR.v1, "cfo left gamma"=gam1, "cfo left ratio"=OR.v1/gam1))
      if (OR.v1>gam1){
        return(1)
      }else{
        return(2)
