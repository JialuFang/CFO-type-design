...
make.decision.ACFO.fn <- function(phi, tys, tns, alp.prior, bet.prior, tover.doses, cidx, diag=FALSE){
  ...
  if (cover.doses[2] == 1){
    ...
    if (is.na(cys[1]) & (cover.doses[3]==1)){
      ...
      #print(data.frame("acfo right OR"=OR.v2, "acfo right gamma"=gam2, "acfo right ratio"=OR.v2/gam2))
      gam2.or <- optim.gamma.fn(cns[2], cns[3], phi, "R", alp.prior, bet.prior)$gamma 
      OR.v2.or <- OR.values(phi, cys[2], cns[2], cys[3], cns[3], alp.prior, bet.prior, type="R")
      #print(data.frame("cfo right OR"=OR.v2.or, "cfo right gamma"=gam2.or, "cfo right ratio"=OR.v2.or/gam2.or))
      if (OR.v2>gam2){
        return(3)
      }else{
        return(2)
      ...
      #print(data.frame("acfo left OR"=OR.v1, "acfo left gamma"=gam1, "acfo left ratio"=OR.v1/gam1))
      gam1.or <- optim.gamma.fn(cns[1], cns[2], phi, "L", alp.prior, bet.prior)$gamma 
      OR.v1.or <- OR.values(phi, cys[1], cns[1], cys[2], cns[2], alp.prior, bet.prior, type="L")
      #print(data.frame("cfo left OR"=OR.v1.or, "cfo left gamma"=gam1.or, "cfo left ratio"=OR.v1.or/gam1.or))
      if (OR.v1>gam1){
        return(1)
      }else{
        return(2)
